import React from 'react';
import PropTypes from 'prop-types';
import {
  View, StyleSheet, Animated, Platform, UIManager,
  TouchableOpacity, Text
} from 'react-native';
import {
  Entypo,
  MaterialIcons
} from "@expo/vector-icons";
import Events from 'react-native-simple-events';
import MapView from 'react-native-maps';
import AutoCompleteInput from './AutoCompleteInput';
import * as Location from "expo-location";
import { create } from "apisauce";

const PLACE_DETAIL_URL = 'https://maps.googleapis.com/maps/api';
const DEFAULT_DELTA = { latitudeDelta: 0.015, longitudeDelta: 0.0121 };

const api = create({
  baseURL: PLACE_DETAIL_URL,
})
let selected=false;
let isSelecting=false;
let current=true;
export default class LocationView extends React.Component {
  static propTypes = {
    apiKey: PropTypes.string.isRequired,
    initialLocation: PropTypes.shape({
      latitude: PropTypes.number,
      longitude: PropTypes.number,
    }).isRequired,
    markerColor: PropTypes.string,
    myLocationBackground: PropTypes.string,
    actionTextStyle: Text.propTypes.style,
    actionText: PropTypes.string,
    onLocationSelect: PropTypes.func,
    debounceDuration: PropTypes.number,
    components: PropTypes.arrayOf(PropTypes.string),
    timeout: PropTypes.number,
    maximumAge: PropTypes.number,
    enableHighAccuracy: PropTypes.bool
  };

  static defaultProps = {
    markerColor: 'black',
    actionText: 'DONE',
    onLocationSelect: () => ({}),
    debounceDuration: 300,
    components: [],
    timeout: 15000,
    maximumAge: Infinity,
    enableHighAccuracy: true
  };

  constructor(props) {
    super(props);
    if (Platform.OS === 'android') {
      UIManager.setLayoutAnimationEnabledExperimental && UIManager.setLayoutAnimationEnabledExperimental(true);
    }
  }

  componentDidMount() {
    Events.listen('InputBlur', this.constructor.displayName, this._onTextBlur);
    Events.listen('InputFocus', this.constructor.displayName, this._onTextFocus);
    Events.listen('PlaceSelected', this.constructor.displayName, this._onPlaceSelected);
  }

  componentWillUnmount() {
    Events.rm('InputBlur', this.constructor.displayName);
    Events.rm('InputFocus', this.constructor.displayName);
    Events.rm('PlaceSelected', this.constructor.displayName);
  }

  state = {
    inputScale: new Animated.Value(1),
    inFocus: false,
    region: {
      ...DEFAULT_DELTA,
      ...this.props.initialLocation,
    },
  };

  _animateInput = () => {
    Animated.timing(this.state.inputScale, {
      toValue: 1,
      duration: 300,
      useNativeDriver: true
    }).start();
  };

  _onMapRegionChange = region => {
    if(isSelecting) {
      this._setRegion(region, false);
    }
    if (this.state.inFocus) {
      this._input.blur();
    }
  };

  _onMapRegionChangeComplete = region => {
    this._input.fetchAddressForLocation(region);
  };

  _onTextFocus = () => {
    this.state.inFocus = true;
    this._animateInput();
    current=false;
    selected = false;
  };

  _onTextBlur = () => {
    this.state.inFocus = false;
    this._animateInput();
  };

  _setRegion = (region, animate = true) => {
    this.state.region = { ...this.state.region, ...region };
    if (animate) this._map.animateToRegion(this.state.region);
  };

  _onPlaceSelected = placeId => {
    this._input.blur();
    isSelecting = true;
    api.get("/place/details/json", { key: this.props.apiKey, placeid: placeId })
        .then(response => {
          let region = (({ lat, lng }) => ({ latitude: lat, longitude: lng }))(
              response.data.result.geometry.location
          );
          this._setRegion(region);
          this.setState({ placeDetails: response.data.result });
        })
    selected = true;
    isSelecting = false;
    current=false;
  };

  _getCurrentLocation = async () => {
    const { timeout, maximumAge, enableHighAccuracy } = this.props;
    let { status } = await Location.requestForegroundPermissionsAsync();
    if (status !== "granted") {
      console.log("no permission");
      return;
    }
    let location = await Location.getCurrentPositionAsync({});
    this._setRegion({ latitude: location.coords.latitude, longitude: location.coords.longitude });
  };

  _validateAddress = async () => {
    if(selected || current) {
      this.props.onLocationSelect({
        ...this.state.region,
        address: this._input.getAddress(),
        placeDetails: this.state.placeDetails
      })
    }
  }

  render() {
    let { inputScale } = this.state;
    return (
      <View style={styles.container}>
        <MapView
          ref={mapView => (this._map = mapView)}
          style={styles.mapView}
          region={this.state.region}
          showsMyLocationButton={true}
          showsUserLocation={false}
          onPress={({ nativeEvent }) => this._setRegion(nativeEvent.coordinate)}
          onRegionChange={this._onMapRegionChange}
          onRegionChangeComplete={this._onMapRegionChangeComplete}
        />
        <Entypo
          name={'location-pin'}
          size={30}
          color={this.props.markerColor}
          style={{ backgroundColor: 'transparent' }}
        />

        <View style={Platform.OS === 'ios' ?styles.fullWidthContainerIos:styles.fullWidthContainer}>
          <AutoCompleteInput
            ref={input => (this._input = input)}
            apiKey={this.props.apiKey}
            style={[styles.input, { transform: [{ scale: inputScale }] }]}
            debounceDuration={this.props.debounceDuration}
            components={this.props.components}
          />
        </View>
        
        <TouchableOpacity style={[styles.currentLocBtn, { backgroundColor: this.props.myLocationBackground }]} onPress={this._getCurrentLocation}>
          <MaterialIcons name={'my-location'} color={'#144333'} size={30} />
        </TouchableOpacity>
        
        
        <TouchableOpacity style={[styles.actionButton, this.props.actionButtonStyle]} onPress={() => this._validateAddress()}>
          <View>
            <Text style={[styles.actionText, this.props.actionTextStyle]}>{this.props.actionText}</Text>
          </View>
        </TouchableOpacity>
        
        {this.props.children}
      </View>
    );
  }
}
const styles = StyleSheet.create({
  container: {
    flex: 1,
    justifyContent: 'center',
    alignItems: 'center',
  },
  mapView: {
    ...StyleSheet.absoluteFillObject,
  },
  fullWidthContainerIos: {
    position: 'absolute',
    width: '90%',
    top: 0,
    alignItems: 'center',
    paddingLeft:25
  },
  fullWidthContainer: {
    position: 'absolute',
    width: '85%',
    top: 35,
    alignItems: 'center',
    paddingLeft:15
  },
  input: {
    width: '100%',
    paddingTop: 5,
  },
  currentLocBtn: {
    height: 50,
    width: 50,
    borderRadius: 50,
    padding: 10,
    alignItems: "center",
    justifyContent: "center",
    backgroundColor: "#fff",
    position: 'absolute',
    bottom: 120,
    right: 10,
  },
  actionButton: {
    backgroundColor: '#000',
    height: 50,
    position: 'absolute',
    bottom: 30,
    left: 33,
    right: 10,
    justifyContent: 'center',
    alignItems: 'center',
    borderRadius: 5,
    width: "85%",
    borderRadius: 30,
  },
  actionText: {
    color: 'white',
    fontSize: 22,
    fontFamily: "beviet-regular"
  },
});
